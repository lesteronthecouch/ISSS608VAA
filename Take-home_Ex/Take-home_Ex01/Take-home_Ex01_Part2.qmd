...
The combination of the box plot clearly illustrates the sge strucutre of each region. The color of each box plot are akso pretty contrast to better tell the difference among regions. 
However, the font zise of the graph are too small to read since the size of the box plot is so much greater than the font.
```{r}
expanded_data <- data %>%
  filter(!is.na(PA)) %>%
  mutate(Age = ifelse(Age == "90_and_Over", "95", Age),
